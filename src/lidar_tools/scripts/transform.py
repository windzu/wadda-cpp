...
LastEditTime: 2023-04-03 02:22:56
Description: 
...
def write_bag(calib_dict,input_bag, output_bag,topic):

    transform=calib_dict[topic]

    # transform the point cloud and write to bag
    with rosbag.Bag(output_bag, 'w') as outbag:
        for cur_topic, msg, t in rosbag.Bag(input_bag).read_messages():
            if cur_topic == topic:
                ...
                outbag.write("ldar_test", cloud_msg, t)
            else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--topic', type=str, default='/lidar_points/top', help='Topic to transform')
    ...
    print('Topic to transform: ', args.topic)   

    calib_dict = read_calib_file(args.calib)
    write_bag(calib_dict,args.input, args.output,args.topic)
